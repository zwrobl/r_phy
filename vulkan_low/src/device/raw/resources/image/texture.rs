    ...
    device::{
        ...
        raw::{
            ...
            resources::{
                ...
                image::{
                    sampler, Image, ImagePartial, ImageRaw, ImageType, Linear, Sampler,
                    SamplerCreateInfo, SamplerRaw,
                },
                Resource,
            },
            Partial,
...
impl<V: ImageType, R: ImageReader<Type = V>> Partial for TexturePartial<V, R> {
    #[inline]
    fn register_memory_requirements<B: crate::device::raw::allocator::AllocatorBuilder>(
        &self,
        builder: &mut B,
    ) {
