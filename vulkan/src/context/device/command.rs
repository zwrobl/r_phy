...
use crate::context::{device::raw::resources::buffer::Buffer, error::{VkError, VkResult}};
...
impl<'a, T, L: Level, O: Operation> RecordingCommand<'a, T, L, O> {
    ...
    pub fn copy_buffer<'b, 'c, S: MemoryProperties, D: MemoryProperties>(
        ...
        let dst = dst.into();
        unsafe {
            device.cmd_copy_buffer(L::buffer(&command.data), src.get_vk_buffer(), dst.get_vk_buffer(), ranges);
        }
        RecordingCommand(command, device)
    }
