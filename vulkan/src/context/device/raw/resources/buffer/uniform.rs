...
use crate::context::{
    device::{
        ...
        raw::{
            ...
            resources::buffer::{
                persistent::PersistentBuffer, Buffer, BufferInfo, BufferInfoBuilder, BufferRaw,
            },
        },
    },
    error::ResourceError,
...
impl<U: AnyBitPattern, O: Operation> UniformBufferInfoBuilder<U, O> {
    ...
    #[inline]
    pub fn build(self, context: &Context) -> UniformBufferInfo<U, O> {
        UniformBufferInfo {
            len: self.len.unwrap(),
...
impl<U: AnyBitPattern, O: Operation> UniformBufferInfo<U, O> {
    #[inline]
    fn get_buffer_info<'a>(&'a self) -> BufferInfo<'a, HostCoherent> {
...
impl<U: AnyBitPattern, O: Operation> Create for UniformBuffer<U, O> {
    type Config<'a> = (UniformBufferInfo<U, O>, AllocatorIndex);
    ...
    fn create<'a, 'b>(
        ...
        let buffer = PersistentBuffer::create((config.get_buffer_info(), allocator), context)?;
        Ok(UniformBuffer {
            buffer,
            _phantom: PhantomData,
        })
...
impl<U: AnyBitPattern, O: Operation> Index<usize> for UniformBuffer<U, O> {
    ...
    fn index(&self, index: usize) -> &Self::Output {
        debug_assert!(
            index * size_of::<U>() < self.buffer.get_size(),
            "Out of range UniformBuffer access!"
        );
